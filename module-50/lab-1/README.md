...
```
[+] Running 2/2
 ✔ Network lab-1_default            Created
 ✔ Container auth_lab1_postgres     Started
```
...
**Verify installation:**
```bash
pip list | grep -E "fastapi|passlib|sqlalchemy"
```
...
Also, add this at the top of `env.py` after imports (around line 8):
```python
from dotenv import load_dotenv
import os
...
**Test health check:**
```bash
curl -X GET "<Load Balancer URL>/ping"
```
...
BASE_URL = "http://127.0.0.1:8000"  # Or use Load Balancer URL
...
if __name__ == "__main__":
    print("=== Testing FastAPI Auth Lab 1 ===\n")

    # Test 1: Health check
    test_health_check()

    # Test 2: Register valid user
    test_register_user("test1@example.com", "SecurePass123!")

    # Test 3: Register another user
    test_register_user("test2@example.com", "AnotherPass456!")

    # Test 4: Try duplicate email (should fail)
    test_register_user("test1@example.com", "DifferentPass789!")

    # Test 5: Try short password (should fail)
    test_register_user("test3@example.com", "short")

    # Test 6: Try invalid email (should fail)
    test_register_user("notanemail", "ValidPass123!")
```
...
**Expected output:**
```
=== Testing FastAPI Auth Lab 1 ===

Health Check: 200
{'status': 'ok', 'message': 'pong'}

Register test1@example.com: 201
{'id': 1, 'email': 'test1@example.com', 'is_active': True}

Register test2@example.com: 201
{'id': 2, 'email': 'test2@example.com', 'is_active': True}

Register test1@example.com: 409
{'detail': 'Email already registered'}

Register test3@example.com: 422
{'detail': [{'loc': ['body', 'password'], 'msg': 'String should have at least 8 characters', 'type': 'string_too_short'}]}

Register notanemail: 422
...
----+----------------------+--------------------------------------------------------------+-----------
  1 | alice@example.com    | $2b$12$LQv3c1yqBWVHxkd0LHAkCOYz6TtxMQJqhN8/LewY5GyE...       | t
  2 | bob@example.com      | $2b$12$kX8Nj9mP3qL5dY7gH2cKzeR4vN6wS8tA9bC1dE2fG3hI...       | t
  3 | david@example.com    | $2b$12$mZ5Qr8pT2uK7eX9jY3fLvbT6sP8xR9nW1vN4mB7cH5kJ...       | t
```
...
If you want to start Lab 2 without implementing Lab 1, you can clone the completed Lab 1 code from our GitHub repository:
